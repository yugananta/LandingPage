...
export default function CommunityFeatures() {
  return (
    <section className="relative py-12 lg:py-16 px-margin-desktop md:px-margin-desktop px-margin-mobile bg-white text-slate-900 border-t border-slate-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-5 order-2 lg:order-1 relative flex justify-center animate-on-scroll scale-up">
            <div className="relative w-[280px] md:w-[320px]">
              ...
              {/* Back shadow/glow */}
              <div className="absolute inset-0 bg-indigo-100/40 rounded-[40px] blur-2xl transform rotate-6 scale-95" />
              
              {/* Phone Frame */}
              <img 
                src="/phone.png" 
                alt="App interface mockup" 
                className="w-full h-auto relative z-10 filter drop-shadow-[0_20px_40px_rgba(15,23,42,0.08)] animate-[float_6s_ease-in-out_infinite]"
              />

              {/* Floating active traders card */}
              <div className="absolute top-1/3 -left-12 bg-white/90 backdrop-blur-md border border-slate-200/80 rounded-2xl p-4 flex items-center gap-3 shadow-xl z-20">
                <div className="w-3 h-3 rounded-full bg-emerald-500 animate-ping" />
                <div>
                  <div className="text-slate-900 font-black text-sm">12K+ Traders</div>
          ...
          <div className="lg:col-span-7 order-1 lg:order-2 flex flex-col gap-6 text-left animate-on-scroll fade-in-up delay-100">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-indigo-200 bg-indigo-50 text-indigo-600 text-xs font-bold uppercase tracking-widest w-fit">
            ...
            {/* Stats Badge Row */}
            <div className="flex gap-4 items-center bg-slate-50 border border-slate-200/80 rounded-xl px-4 py-2.5 w-fit text-xs font-semibold text-slate-700">
              <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-indigo-600" />45+ Active Cities</span>
              <span className="text-slate-200">|</span>
              <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-indigo-600" />12K+ Active Members</span>
            </div>
              ...
              {FeaturesRow.map((f, i) => (
                <div key={i} className="flex gap-3 items-start">
                  <div className="w-9 h-9 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-center shrink-0">
                    <span className="material-symbols-outlined text-indigo-600 text-lg">{f.icon}</span>
                  </div>
                  <div>
                    <div className="text-slate-900 font-bold text-xs">{f.title}</div>
